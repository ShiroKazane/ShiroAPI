    ...
    var _ = (Prism.Live = class PrismLive {
        constructor(source) {
                ...
                keydown: (evt) => {
                    if (evt.key == 'Tab' && !evt.altKey) {
                        ...
                        if (this.tabstops && this.tabstops.length > 0) {
                            ...
                            var selection = _.adjustIndentation(
                                this.selection,
                                {
                                    relative: true,
                            ...
                            if (outdent) {
                                ...
                                this.selectionStart +=
                                    before.length +
                                    1 -
                                    (outdent + isBeforeIndented);
                            } else {
                                // Indent
                                var hasLineAbove =
                                    before.length == this.selectionStart;
                                this.selectionStart +=
                                    before.length + 1 + !hasLineAbove;
                            }
                        } else {
            ...
            requestAnimationFrame(() => {
                ...
                this.pre.style.height =
                    ...
                    sourceCS.getPropertyValue('--height');
                this.pre.style.maxHeight =
                    this.source.style.maxHeight ||
                    sourceCS.getPropertyValue('--max-height');
                this.textarea.spellcheck =
                    this.source.spellcheck ||
                    sourceCS.getPropertyValue('--spellcheck');
            });

            this.update();
            this.lang = (this.code.className.match(/lang(?:uage)?-(\w+)/i) || [
                ,
            ])[1];
            ...
            this.source.dispatchEvent(
                new CustomEvent('prism-live-init', {
                    bubbles: true,
                    detail: this,
                })
            );
        ...
        expandSnippet(text) {
            ...
            if (expansion) {
                ...
                while ((match = _.CARET_INDICATOR.exec(str))) {
                    ...
                    replacement.push(
                        str.slice(0, match.index + match[1].length)
                    );
        ...
        syncStyles() {
            ...
            for (var prop in cs) {
                if (
                    cs[prop] &&
                    prop in this.textarea.style &&
                    properties.test(prop)
                ) {
                    this.wrapper.style[prop] = cs[prop];
                    this.textarea.style[prop] = this.pre.style[prop] =
                        'inherit';
                }
            }
        ...
        toggleComment() {
            ...
            if (commentNode) {
                ...
                if (
                    ...
                    this.set(
                        this.value.slice(
                            start + comments.singleline.length,
                            end
                        ),
                        { start, end }
                    );
                    ...
                    this.set(
                        this.value.slice(start + comments[0].length, end),
                        { start, end: end + comments[1].length }
                    );
                }
            } else {
        ...
        delete(characters, { forward, pos } = {}) {
            ...
            if (pos) {
                ...
                this.selectionEnd =
                    this.selectionEnd - pos + this.selectionStart;
            }
        }
